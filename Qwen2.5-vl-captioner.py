...
class ProcessConfig:
    ...
    output_format: str = "csv"  # New field: can be "csv" or "individual"
    output_file: str = "video_captions.csv"
    csv_delimiter: str = "|"
...
class VideoCaptioner:
    ...
    def generate_caption(self, frame_paths: List[Path]) -> str:
        """Generate caption for video frames."""
        messages = self._create_messages(frame_paths)
        ...
        with torch.no_grad():
            # Ne használjunk generator paramétert, helyette állítsuk be a torch seed-et
            ...
            generated_ids = self.model.generate(
                **inputs,
                ...
                # generator paramétert eltávolítottuk
            )
    ...
    def process_video(self, video_path: Path) -> dict:
        ...
        try:
            caption = self.generate_caption(frame_paths)
...
def process_videos(config: Config):
    ...
    # If using CSV format, create a fresh file (clear any existing content)
    if config.process.output_format == "csv":
        output_file = output_dir / config.process.output_file
        output_file.unlink(missing_ok=True)
    
    captioner = VideoCaptioner(config)
    processed_count = 0
    ...
    def process_single_video(video_path: Path):
        ...
        try:
            ...
            print(f"\nProcessing video: {rel_path}")
            
            result = captioner.process_video(video_path)
            result_dict = {
                'path': str(rel_path),
    ...
    if input_path.is_dir():
        ...
        if input_path.suffix.lower() in SUPPORTED_VIDEO_FORMATS:
            process_single_video(input_path)
        else:
            print(f"Unsupported file format: {input_path}")
            print(f"Supported formats: {SUPPORTED_VIDEO_FORMATS}")
